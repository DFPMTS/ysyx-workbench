...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc, s->dnpc = s->pc + imm, ftrace_log(s, rd, -1, imm));
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, R(rd) = s->snpc, s->dnpc = (src1 + imm) & (~1), ftrace_log(s, rd, rs1, imm));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I_SHIFT, R(rd) = src1 << imm);  
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R_SHIFT, R(rd) = src1 >> src2);
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R_SHIFT, R(rd) = (int32_t)src1 >> src2);
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(rd) = src1 & src2);
  // TODO                                           fence
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(11, s->pc)); // ecall from M-mode
