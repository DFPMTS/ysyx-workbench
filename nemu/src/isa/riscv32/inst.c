...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R_SHIFT, R(rd) = src1 << src2);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I_SHIFT, R(rd) = (int32_t)src1 >> imm);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I_SHIFT, R(rd) = src1 >> imm);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bgeu   , B, s->dnpc = (src1 >= src2) ? (s->pc + imm) : s->snpc);
